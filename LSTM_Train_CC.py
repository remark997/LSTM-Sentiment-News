...
import os
os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'   # for seaborn installation
...
#  Neural network module backup by TrensorFlow
...
def remove_stopword_titles(titles):
    for i, title in enumerate(titles):
        titles[i]=_remove_stopwords(title)
    return(titles)
...
plt.savefig('Word frequency Histogram_PhraseBank.png')
plt.show()
...
# data['text'] = data['text'].apply(lambda x: _remove_stopwords(x))   # performace worses if removing stopwords
...
data.sentiment.value_counts()/2263
# neutral     0.614229
# positive    0.251878
# negative    0.133893
...
Y = pd.get_dummies(data['sentiment']).values   # convert string into [0, 1, 0](neutral), [0, 0, 1](positive), [1, 0, 0](negative)
...
#Tokenization
max_fatures = len(list(data["text"].str.split(' ', expand=True).stack().unique())) + 1 #Number of unique words 5500
...
# Top 5 most common words are: [('the', 2437), ('of', 1393), ('in', 1248), ('and', 1026), ('to', 972)]
# Found 5170 unique tokens.
# after removing stopwords Top 5 most common words are: [('eur', 876), ('mn', 448), ('company', 308), ('profit', 299), ('net', 269)]
# Found 5015 unique tokens
...
## pad: make training sentence the same length (N: batchsize, M:longest length)
...
le = LabelEncoder()   # Encode target labels with value between 0 and n_classes-1, target to Y, not X
...
# Shape of train set: (1832, 60)
# Shape of validation set: (204, 60)
...
# filepath_a = "Naive_stopwords.hdf5"
...
Y_ints = [Y.argmax() for Y in y_train_emb]
class_weights = {0: (1.3*1387)/303.,1: 1.,2: 1387/569.}
class_weights = {0: 4.59 ,1: 1.,2: 2.44} # in Alex's thesis

filepath_c = "SMOTE_4.hdf5"
...
path = "/Users/cathychen/OneDrive/NAS Backup/DeepLearning/AlexDoebelea"
os.chdir(path)
...
# filepath_c = "pretrain_wordem_model.hdf5"
# filepath_c = "Word2Vec.hdf5"

filepath_c = "SMOTE_4.hdf5"
filepath_a = "Naive.hdf5"
filepath_b = "Classweights.hdf5"
model = load_model(filepath_b, custom_objects={"precision": precision, "recall":recall})
...
# tn, fp, fn, tp = confusion_matrix(y_test_le, prediction).ravel()
